...
构建脚本核对游戏指纹、发布日期、完整签名、读取器、上限、控制钩子及内嵌计划，然后更新自有 DLL 和哈希，不部署或启动游戏。游戏内由使用者验证三档倍率、四类点数归属、道具嵌套、金钱消费、正常收入及保存加载。部署通过启动器事务安装，安装与更新直接尝试；若文件被占用，关闭目标游戏后重试。运行中的游戏需重启才能加载新模组。
