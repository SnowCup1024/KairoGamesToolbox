...
Windows x64 桌面启动器，当前版本 **0.2.3**。支持 Steam 开罗游戏与手动添加的非 Steam 游戏；内置 63 款游戏的中英文名称和拼音索引。启动器、模组包和本地游戏文件分别管理，启动器发布 ZIP 仅包含一个 EXE。
...
.\RunLauncher.bat
.\Scripts\BuildRelease.ps1 -Version 0.2.3 -SkipLaunchCheck
```

本轮不使用 computer use，不自动或隐藏启动游戏。构建后由用户手动验证界面，因此跳过发布脚本的窗口启动检查。发布输出在 `.Build/Publish/v0.2.3` 和 `.Build/Package`，保留的 v0.2.1 EXE 可用于对照。
...
0.2.3 手动验收重点：标题没有“· 修改”；修改菜单只显示金钱反加及开关，悬浮文字出现介绍；未连接时约每 15 秒重试，连接后等待至少 150 秒确认控件不闪烁；手动刷新和切换开关仍可立即操作，断线后显示未知并禁用控制。
