...
面向 Steam 开罗游戏的 Windows x64 桌面启动器，当前版本为 `0.2.1`。
...
```powershell
.\Scripts\BuildRelease.ps1 -Version 0.2.1
```
...
- 游戏详情右侧新增“进入修改页面”，针对所选游戏目录连接运行中的模组。
- 金钱反加开关从游戏端读取状态，设置收到确认后才显示生效；离线显示未知，不把本地开关值当作已应用。
- 首次控制需先进入存档，再点击“我已进入存档 · 启用控制”，避免过早初始化游戏类型。
- 本机命名管道协议 v1：按 AppID 和规范化目录哈希命名，仅当前用户访问；单次请求有超时，回复校验目标身份。
- 实时控制需要本地测试模组 0.0.4。该模组仅在 KairoMods 构建部署，不提交、不打包；仓库现有 0.0.3 ZIP 不变，不能用于此功能。
- 每次游戏启动默认关闭；关闭启动器或离开修改页不会撤销游戏端开关。关闭开关不会回滚已经改变的余额或存档。
- 112 项启动器自动测试及 9 项本地模组计算/通信测试通过。未自动启动游戏，真实消费和界面由用户验证。
