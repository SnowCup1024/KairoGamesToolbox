...
Windows x64 桌面启动器，当前版本 **0.2.4**。支持 Steam 开罗游戏与手动添加的非 Steam 游戏；内置 63 款游戏的中英文名称和拼音索引。启动器、模组包和本地游戏文件分别管理，启动器发布 ZIP 仅包含一个 EXE。

## 0.2.4 更新

- 删除独立模组控制页面，第 03 张卡片直接显示当前游戏支持的修改项；连接状态和无边框刷新图标位于卡片标题右侧，窄窗口自动换行。
- 功能按 AppID 定义在 `ModFeatures`。当前只有铜锣烧店的金钱反加，未适配游戏显示暂无修改项。名称紧邻开关，悬浮名称查看介绍；不显示其他游戏的通用假开关。
- 网格按接近 4:3 的列/行数量排列，例如 12 项宽屏时为 4 列×3 行。根据实际宽度与每项最小占用宽度减少列数，最少 1 列。
- 自动与手动连接请求均要求目标目录有游戏和已知控制插件文件；仅残留安装记录不能连接。未安装时刷新按钮禁用，定时器只检查本地文件，不发送管道请求。未连接每 15 秒检查，连接后每 150 秒检查，切页与切换目录取消旧请求。
- 首次点击修改开关且游戏端尚未启用时，弹窗确认已进入存档，再依次启用控制和发送修改命令。刷新按钮只查询状态，不提前挂钩游戏方法。
- 统一游戏库、详情、设置、关于的文字层级：主标题 28、分区标题 20、正文和标签 14、辅助版本文字 12；仅标题和标签加重。
- 页面宽度受可见区域约束，最大 1050，两侧至少预留 28；长路径换行，窄屏目录按钮纵向排列，修复卡片右侧被裁切的问题。
- 导航与详情增加 180 毫秒渐入和轻微位移，遵守系统动画设置；使用桌面亚克力背景，系统不支持时回退至 Mica，透明效果由系统设置决定。旧版本中遮挡系统背景的不透明页面底色已移除。

## 0.2.3 热补丁：单实例

- 同一 Windows 会话内使用固定命名互斥量限制一个启动器实例；标识不包含版本号、EXE 名称或路径，后续版本共用。
- 重复启动在初始化设置和主窗口前退出，并尝试恢复、激活已有窗口。首次实例退出或异常终止后可以重新打开。
- 兼容检测原文件名的历史版本窗口：先打开旧版再打开热补丁时，热补丁退出并切回旧版。未更新的历史 EXE 不参与互斥机制，因此反过来再运行旧版不能由热补丁阻止；不强行结束旧进程。所有启动入口使用本热补丁或后续版本，才能完整保证跨版本单实例。
- 版本号仍为 0.2.3。本轮原版正在运行，产物输出到 `.Build/Publish/v0.2.3-hotfix1`，不覆盖正在使用的 EXE。

## 0.2.3 更新

- 修改页标题仅显示游戏名；功能卡片改为“修改菜单”，金钱反加仅保留名称和开关，悬浮名称显示功能简介。
- 进入页面立即检查连接；未连接时每 15 秒重试，收到有效回复后每 150 秒检查。连接成功但尚未启用控制也使用 150 秒间隔。手动刷新、启用和切换开关直接发送请求。
- 后台状态检查不再临时禁用开关，状态未变时不重置开关；操作请求串行执行，避免轮询期间点击被丢弃。离开页面停止轮询并取消等待。
- 连接检查是定时轮询，不是主动推送。游戏退出后，页面最迟到下一次检查才发现断线；开关命令仍独立等待游戏确认，未收到确认会提示结果未知。
- Mods 仅跟踪 Source 源码和随源码维护的说明；删除仓库当前版本的 0.0.3 ZIP 与空发布目录，新增忽略规则。模组打包脚本输出改到 `.Build/ModPackages`，不在本轮制作模组包。
...
完整 ZIP 格式见 [Mods/Source/README.md](Mods/Source/README.md)。当前仓库只保留模组源码；0.0.4 源码支持金钱反加，尚未打包发布。

## 实时控制原理

详情页第 03 区直接操作修改项。先安装模组并启动游戏，进入存档后操作金钱反加开关；首次使用按弹窗确认已进入存档。不要在主菜单激活游戏方法。
...
.\RunLauncher.bat
.\Scripts\BuildRelease.ps1 -Version 0.2.4 -SkipLaunchCheck
```

本轮不使用 computer use，不自动或隐藏启动游戏。构建后由用户手动验证界面，因此跳过发布脚本的窗口启动检查。发布输出在 `.Build/Publish/v0.2.4` 和 `.Build/Package`，旧版 EXE 保留用于对照。
...
0.2.2 已验证 128 项启动器测试、9 项模组计算/通信测试及隔离副本中的旧版安装识别。0.2.3 不修改模组逻辑，发布脚本重新执行启动器测试；界面与真实游戏行为由用户手动验收。

0.2.4 手动验收重点：1:1 窗口下卡片两侧留白；各页面标题与文字层级一致；切页动画与浅色/深色亚克力背景；未安装 Mod 时刷新禁用，安装后可查询；未适配游戏无金钱反加；首次修改出现进入存档确认，操作有游戏端确认；等待至少 150 秒核对开关不闪烁。自动测试覆盖合成的 12 项网格布局，当前实际游戏只有一个功能，不添加演示开关。
